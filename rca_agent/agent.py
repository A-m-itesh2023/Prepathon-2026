...
class RCAAgent:
    ...
    def investigate(self, incident):
        ...
        for h in self.reasoner.initial_hypotheses(incident):
            state.hypotheses[h.name] = h
        for _ in range(self.max_iterations):
            state.iterations += 1
            query = self.reasoner.next_query(state)
            if not query:
                break
        ...
        ranked = sorted(state.hypotheses.values(), key=lambda x: x.score, reverse=True)
        top = ranked[0] if ranked else None
        confidence = self._confidence(top.score if top else 0, len(state.evidence))
        if not top or top.score <= 0:
            conclusion = "Insufficient evidence to identify a defensible root cause."
            confidence = "low"
        else:
            conclusion = top.name
        return {
            ...
            "observed_facts": [e.observation for e in state.evidence],
            "evidence": [e.__dict__ for e in state.evidence],
            "hypotheses": [h.__dict__ for h in ranked],
            "timeline": state.timeline,
            "alternative_explanations": [h.name for h in ranked[1:3]],
    ...
    def _execute(self, query):
        ...
        if query == "logs":
            pods = self.gateway.pods().data
            if isinstance(pods, list) and pods:
                return self.gateway.logs(pods[0]["name"])
            return self.gateway.logs("unknown")
        if query == "metrics":
            return self.gateway.metrics("http://prometheus.monitoring.svc:9090", "up")
